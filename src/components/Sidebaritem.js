...
const Sidebaritem = (props) => {
  ...
  return (
    <div
      className={`${styles.sidebaritem_container}
		${isActive && !isCollapsed ? styles.sidebaritem_container_active : ""}`}
      onMouseEnter={props.onMouseEnter}
      onMouseLeave={props.onMouseLeave}
      id={props.id}
    >
      <DefaultButton
        className={`${styles.sidebar_item_btn} 
			${isActive && isCollapsed ? styles.sidebar_item_btn_active : ""}
			${
        isActive && (!isContentOpen || singleItem)
          ? styles.sidebar_item_btn_active
          : ""
      }`}
      ...
      >
        <div className={styles.sideitem_container}>
          ...
          {!isCollapsed && <div className={styles.sideitem_title}>{title}</div>}
        </div>
      </DefaultButton>

      {isContentOpen && isActive && !isCollapsed && (
        <div className={styles.sideitem_content}>{children}</div>
      )}
      {isCollapsed && callout}
    </div>
  );
